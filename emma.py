...
def travel():
    ...
    cont ={'中文':["列举%s所有的国家"%(continent),"你要去哪个国家旅游？","国家","你要去哪个地区旅游？","地区"],
    'English':['List all countries in %s'%(continent),"Which country are you traveling to?","Country","Which area are you visiting?","Area"]}
    
    st.button("OK",on_click=choose_continent)
    if st.session_state.continent:
        ...
        st.button("OK",on_click=choose_country)
        ...
        if st.session_state.country and country:
            ...
            st.button("OK",on_click=choose_area)
